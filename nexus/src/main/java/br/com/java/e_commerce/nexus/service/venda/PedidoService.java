...
>>>>>>> cc85b8d9e8047f09ba782373ee5397cd4b3cf4ab
    ...
    public PedidoService(PedidoRepository pedidoRepository,
        ...
        this.devolucaoService = devolucaoService;  // ADICIONADO
    }
    ...
    /**
     * Obtém o status a ser exibido para o cliente.
     *
     * ⭐ NOVO MÉTODO: Se o pedido está EM_ABERTO mas já possui pagamento aprovado,
     * exibe AGUARDANDO_PAGAMENTO (Aguardando confirmação) em vez de EM_ABERTO.
     *
     * @param pedido Pedido a ser verificado
     * @return StatusPedido apropriado para exibição
     */
    public StatusPedido getStatusExibicao(Pedido pedido) {
        if (pedido.getStatus() == StatusPedido.EM_ABERTO
                && pedido.getPagamento() != null
                && pedido.getPagamento().getStatus() == StatusPagamento.APROVADO) {
    ...
    /**
     * Verifica se o pedido pode ser pago pelo cliente.
     * Um pedido pode ser pago se está EM_ABERTO e NÃO tem pagamento aprovado.
     *
     * @param pedido Pedido a ser verificado
     * @return true se pode pagar, false caso contrário
     */
    public boolean podePagar(Pedido pedido) {
        return pedido.getStatus() == StatusPedido.EM_ABERTO
                && (pedido.getPagamento() == null
                || pedido.getPagamento().getStatus() != StatusPagamento.APROVADO);
    }
     ...
     * Um pedido pode ser cancelado se está EM_ABERTO e NÃO tem pagamento aprovado.
     *
     * @param pedido Pedido a ser verificado
...
<<<<<<< HEAD
     ...
     * @return Cupom de troca
     * @throws UnsupportedOperationException sempre, pois o método foi movido para DevolucaoService
     */
    @Deprecated
    @Transactional
...
<<<<<<< HEAD
     ...
     * @return Lista de pedidos do cliente com status sincronizados com devoluções
     */
    @Transactional(readOnly = true)
    public List<Pedido> listarPorCliente(Long clienteId) {
        List<Pedido> pedidos = pedidoRepository.findByClienteIdWithItens(clienteId);

        // Para cada pedido, verifica se existe solicitação de devolução e atualiza o status
        for (Pedido pedido : pedidos) {
            boolean temPendente = devolucaoService.possuiSolicitacaoPendente(pedido);
            pedido.setTemSolicitacaoPendente(temPendente);

            // Busca a última solicitação para este pedido
            Optional<SolicitacaoDevolucao> solicitacao = devolucaoService.buscarUltimaPorPedido(pedido);

            if (solicitacao.isPresent()) {
                StatusSolicitacao statusSolicitacao = solicitacao.get().getStatus();

                // Atualiza o status do pedido com base no status da solicitação
                switch (statusSolicitacao) {
                    ...
                    case TROCA_AUTORIZADA:
                        ...
                        break;
                    case CONCLUIDA:
                        pedido.setStatus(StatusPedido.DEVOLVIDO);
                        break;
                    case RECUSADA:
                        // Mantém o status original (ENTREGUE)
                        break;
                    case CANCELADA:
                        // Mantém o status original (ENTREGUE)
                        break;
